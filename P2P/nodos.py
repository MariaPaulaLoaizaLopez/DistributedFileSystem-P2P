...
def ConexionConCliente(msg,nodo):
    ...
    if opcion=='upload': 
        ...
        if responsable(nodo.rango, int(llaveArchivo)):
            ...
            recibirArchivo(archivo_encode,llaveArchivo,nodo.myAddr,nodo.id)
    ...
    if opcion=='download':
        ...
        enviarArchivo(nombreArchivo,llaveArchivo,nodo.id) 
...
def enviarArchivo(nombreArchivo,llaveArchivo,nodoId):

    ubicacion = open('servidor'+nodoId+'/'+llaveArchivo, 'rb')
    ...
    myconex.send_multipart([nombreArchivo.encode(),archivo_encode])
